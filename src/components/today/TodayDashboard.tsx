...
const TIMELINE_HOURS = Array.from({ length: 25 }, (_, index) => TIMELINE_START + index * 60)
...
export function TodayDashboard({
  ...
  const timelineCanvasHeight = Math.max(TIMELINE_HEIGHT, panelsHeight - TIMELINE_PANEL_CHROME_HEIGHT)
          ...
          <div className="min-h-0 flex-1 overflow-y-auto scrollbar-thin">
            ...
            >
              ...
              {TIMELINE_HOURS.map(minute => {
                const top = ((minute - TIMELINE_START) / (TIMELINE_END - TIMELINE_START)) * timelineCanvasHeight
                return (
              ...
              {isToday && (() => {
                ...
                const top = ((normalizedNow - TIMELINE_START) / (TIMELINE_END - TIMELINE_START)) * timelineCanvasHeight
              ...
              {dragPreviewMinute !== null && (
                <div className="absolute left-0 right-1/2 z-30 border-t-2 border-dashed border-[var(--purple)] pointer-events-none" style={{ top: ((dragPreviewMinute - TIMELINE_START) / (TIMELINE_END - TIMELINE_START)) * timelineCanvasHeight }}>
              ...
              {routineTimelineGroups.map(group => {
                const top = ((group.start - TIMELINE_START) / (TIMELINE_END - TIMELINE_START)) * timelineCanvasHeight
                const height = Math.max(30, ((group.end - group.start) / (TIMELINE_END - TIMELINE_START)) * timelineCanvasHeight)
              ...
              {chronological.map(item => {
                ...
                const top = ((start - TIMELINE_START) / (TIMELINE_END - TIMELINE_START)) * timelineCanvasHeight
                const height = Math.max(30, ((end - start) / (TIMELINE_END - TIMELINE_START)) * timelineCanvasHeight)
