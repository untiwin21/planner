'use client'
import clsx from 'clsx'
import { formatDate } from '@/lib/dates'
import { PLAN_CATEGORY_STYLE, shortGoalCategory } from '@/lib/planCategory'
import { tasksProgress } from '@/lib/taskProgress'
import type { ShortGoal } from '@/types'

interface Props {
  weekDays: Date[]
  goalRows: ShortGoal[][]   // pre-computed rows to avoid overlap
  selectedGoalId: string | null
  onEditGoal: (id: string) => void
}

function dateToColIndex(date: string, weekDays: Date[]): number {
  return weekDays.findIndex(d => formatDate(d) === date)
}

export function GoalSpanRow({ weekDays, goalRows, selectedGoalId, onEditGoal }: Props) {
  if (goalRows.length === 0) return null

  return (
    <div className="flex flex-col gap-1.5 mt-1.5">
      {goalRows.map((row, rowIdx) => (
        <div key={rowIdx} className="grid grid-cols-7 gap-2">
          {buildRowCells(row, weekDays, selectedGoalId, onEditGoal)}
        </div>
      ))}
    </div>
  )
}

function buildRowCells(
  goals: ShortGoal[],
  weekDays: Date[],
  selectedGoalId: string | null,
  onEditGoal: (id: string) => void
) {
  const cells: React.ReactNode[] = []
  let col = 0
  const sorted = [...goals].sort((a, b) => a.date_from.localeCompare(b.date_from))

  for (const goal of sorted) {
    const weekStart = formatDate(weekDays[0])
    const weekEnd = formatDate(weekDays[6])
    const clampedFrom = goal.date_from < weekStart ? weekStart : goal.date_from
    const clampedTo = goal.date_to > weekEnd ? weekEnd : goal.date_to
    const startCol = dateToColIndex(clampedFrom, weekDays)
    const endCol = dateToColIndex(clampedTo, weekDays)
    if (startCol < 0 || endCol < 0) continue

    const span = endCol - startCol + 1
    while (col < startCol) {
      cells.push(<div key={`empty-${col}`} className="col-span-1" />)
      col++
    }

    const progress = tasksProgress(goal.tasks)
    const total = progress.total
    const pct = progress.pct
    const isSelected = selectedGoalId === goal.id
    const meta = PLAN_CATEGORY_STYLE[shortGoalCategory(goal)]

    cells.push(
      <button
        key={goal.id}
        onClick={() => onEditGoal(goal.id)}
        style={{ gridColumn: `span ${span}` }}
        className={clsx(
          'flex items-center gap-3 px-3 py-2 rounded-[10px] border transition-all duration-150 text-left w-full',
          meta.event,
          isSelected ? 'shadow-[0_0_0_2px_rgba(124,92,191,0.22)]' : 'hover:shadow-sm',
        )}
      >
        <div className="w-1 self-stretch rounded-full flex-shrink-0" style={{ background: meta.accent }} />

        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <p className="text-[13px] font-semibold truncate">{goal.title}</p>
            <span className="shrink-0 text-[9px] font-semibold opacity-65">{meta.label}</span>
          </div>
          {total > 0 && (
            <div className="flex items-center gap-2 mt-1">
              <div className="flex-1 h-[2px] rounded-full bg-white/70">
                <div className="h-full rounded-full transition-all duration-500" style={{ width: `${pct}%`, background: meta.accent }} />
              </div>
              <span className="text-[11px] opacity-70 flex-shrink-0 tabular-nums">{pct}%</span>
            </div>
          )}
        </div>
      </button>
    )

    col = endCol + 1
  }

  while (col < 7) {
    cells.push(<div key={`empty-end-${col}`} className="col-span-1" />)
    col++
  }

  return cells
}
