            ...
            {weekDays.map((_, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <span className="text-[9px] text-[var(--text-3)] h-3 flex items-end">{hasData ? `${pct}%` : ''}</span>
            ...
            <div className="flex flex-col gap-2">
              {heatmapRoutines.map(r => (
                <div key={r.id} className="flex items-center gap-2">
                  <span className="text-[11px] text-[var(--text-2)] truncate flex-shrink-0" style={{ width: '100px' }}>{r.name}</span>
        ...
        {showForm && (
          <div className="mb-5 p-4 rounded-[14px] bg-[var(--surface-2)] border border-[var(--border)]">
            ...
            <div className="flex items-center justify-between mt-3">
              <span className="text-[10px] text-[var(--text-3)]">Ctrl+Enter로 저장</span>
