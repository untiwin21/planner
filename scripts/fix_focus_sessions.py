...
p = Path('src/types.ts')
s = p.read_text()
needle = "export interface Task {\n"
insert = """export interface FocusSessionRecord {
  id: string
  started_at: string
  ended_at: string
  duration_min: number
...
"""
if 'export interface FocusSessionRecord' not in s:
    if needle not in s:
        raise SystemExit('Task interface marker not found')
    s = s.replace(needle, insert, 1)
task_marker = "  actual_status?: 'recorded' | 'skipped'\n  /** Retrospective-only item: shown on the actual timeline, never as a planned task. */"
task_fields = """  actual_status?: 'recorded' | 'skipped'
  ...
  /** Retrospective-only item: shown on the actual timeline, never as a planned task. */"""
if task_marker in s:
    s = s.replace(task_marker, task_fields, 1)
p.write_text(s)

p = Path('src/components/today/TaskExecutionLayer.tsx')
s = p.read_text()
s = s.replace("import type { DayEntry, SubTask, Task } from '@/types'", "import type { DayEntry, FocusSessionRecord, SubTask, Task } from '@/types'")
s = s.replace("""interface FocusSessionRecord {
  id: string
  started_at: string
  ended_at: string
  duration_min: number
...
""", '')
s = s.replace("  accumulatedMs: number\n  running: boolean", "  accumulatedMs: number\n  segments: Array<{ startedAt: number; endedAt: number }>\n  running: boolean", 1)
s = s.replace("    return parsed as ActiveFocusSession", "    return { ...parsed, segments: Array.isArray(parsed.segments) ? parsed.segments : [] } as ActiveFocusSession")
old = """      const next = {
        ...current,
...
if old not in s: raise SystemExit('pause block not found')
s = s.replace(old, new, 1)
old = """      segmentStartedAt: startedAt,
      accumulatedMs: 0,
      running: true,"""
new = """      segmentStartedAt: startedAt,
      accumulatedMs: 0,
      segments: [],
      running: true,"""
if old not in s: raise SystemExit('start block not found')
s = s.replace(old, new, 1)
old = """      const finishedAt = Date.now()
      const activeMinutes = elapsedMs(session, finishedAt) / 60_000
      const days = readDays()"""
new = """      const finishedAt = Date.now()
      ...
      const days = readDays()"""
if old not in s: raise SystemExit('finish start not found')
s = s.replace(old, new, 1)
old = """      const previousMinutes = taskActualMinutes(task) ?? 0
...
new = """      const previousMinutes = taskActualMinutes(task) ?? 0
      const totalMinutes = previousMinutes + activeMinutes
...
if old not in s: raise SystemExit('finish records not found')
s = s.replace(old, new, 1)
s = s.replace("""function addMinutesToClock(clock: string, minutes: number) {
...
s = p.read_text()
s = s.replace("import type { BadgeColor, Category, DayEntry, DayMeta, LongGoal, Routine, RoutineConfig, RoutineLog, RoutineLogPatch, RoutinePeriod, RoutineStatus, ShortGoal, SubTask, Task, TaskScheduleInput } from '@/types'", "import type { BadgeColor, Category, DayEntry, DayMeta, FocusSessionRecord, LongGoal, Routine, RoutineConfig, RoutineLog, RoutineLogPatch, RoutinePeriod, RoutineStatus, ShortGoal, SubTask, Task, TaskScheduleInput } from '@/types'")
old = """interface ActualTimelineItem {
  key: string
  token: string
  task: Task
  subtask?: SubTask
...
new = """interface ActualTimelineItem {
  key: string
  token: string
  task: Task
  subtask?: SubTask
...
if old not in s: raise SystemExit('actual iface not found')
s = s.replace(old, new, 1)
i = s.index("  const actualBlocks = useMemo<ActualTimelineItem[]>(() => {")
j = s.index("\n\n  const flexible = useMemo", i)
block = """  const actualBlocks = useMemo<ActualTimelineItem[]>(() => {
...
for old, new in pairs:
    if old not in s: raise SystemExit('manual actual block not found')
    s = s.replace(old, new, 1)
old = """      actual_start_time: undefined,
      actual_end_time: undefined,
      actual_status: undefined,
    } : { discarded: false })"""
new = """      actual_start_time: undefined,
      actual_end_time: undefined,
      actual_status: undefined,
      actual_duration_min: undefined,
      actual_sessions: undefined,
    } : { discarded: false })"""
if old not in s: raise SystemExit('discard block not found')
s = s.replace(old, new, 1)
old = """  function toggleTaskWithActualEditor(task: Task) {
    onToggleTask(task.id)
    if (task.done || task.discarded || !canEditActual) return"""
new = """  function toggleTaskWithActualEditor(task: Task) {
    onToggleTask(task.id)
